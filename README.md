...
2026-09-14 기준 1·2단계 **설계 문서**를 완료했다. 실행 코드, DB 마이그레이션, 자동화 금융 테스트와 성능 측정은 아직 없다. 문서의 테스트 항목은 검증 계획이며 통과 실적을 의미하지 않는다.

설계 기준은 고객별 일일 이체 한도, PostgreSQL 행 잠금과 영속 멱등성 키, 단일 금융 커밋, 마감 스냅샷이다. Redis는 캐시/세션 확장 후보이며 초기 금융 정확성의 필수 구성요소가 아니다.

다음 작업은 3단계 Java/Spring Boot·Gradle 프로젝트와 PostgreSQL 개발 환경 구성, 마이그레이션, 회원·계좌 및 인증 최소 골격이다. 구체 버전은 부트스트랩 시 호환성을 확인해 고정한다.
